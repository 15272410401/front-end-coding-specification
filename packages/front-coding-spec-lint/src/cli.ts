...
const installDepsIfThereNo = async () => {
  ...
  if (!fsExtra.existsSync(nodeModulesPath) && lintConfigFiles.length > 0) {
...
program
  ...
  .description(
    `${PACKAGE_NAME} 是一个关于前端编码规范的 CLI 工具，用于创建或接入前端项目的编码规范配置，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡点，降低项目实施规范的成本`,
  );
...
program
  ...
  .action(async (cmd) => {
    ...
    if (files) log.warn(`[${PACKAGE_NAME}] changes not staged for commit: \n${files}\n`);
