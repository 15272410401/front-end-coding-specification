...
- 1.2.1.【强制】使用分号。`eslint`: [semi](https://eslint.org/docs/rules/semi)
  ...
  ```javascript
  // 错误 - 导致 Uncaught ReferenceError 报错
  const chenghuai = {}
  const huaicheng = {}
...
- 2.1.2.【强制】正确地使用 `const` 和 `let`。`eslint`: [prefer-const](https://eslint.org/docs/rules/prefer-const)
  ...
  ```javascript
  // 错误 - 声明后未发生重新赋值，应使用 const
  let flag = true;
  if (flag) {
    console.log(flag);
  }

  // 正确 - 声明后发生重新赋值，let 使用正确
  let flag = true;
  if (flag) {
    flag = false;
  }
...
- 2.1.6.【参考】哪里使用，哪里声明。
  ...
  ```javascript
  // 错误 - 如果权限校验（checkUserPermission）失败，fetchData 是不必要的
  function getData(id) {
...
- 2.1.9.【强制】禁止连续赋值。`eslint`: [no-multi-assign](https://eslint.org/docs/rules/no-multi-assign)
  ...
  ```javascript
  // 错误 - 本例的结果是 let 仅对 a 起到了预想效果，b 和 c 都成了全局变量
  (function test() {
...
- 2.3.2.【强制】某些数组方法的回调函数中必须包含 `return` 语句。`eslint`: [array-callback-return](https://eslint.org/docs/rules/array-callback-return)
  ...
  // 错误 - map 应用于构建一个新数组，单纯想遍历数组应使用 forEach
  ...
  // 错误 => Uncaught TypeError: Cannot set property 'b' of undefined
...
- 2.4.9.【强制】不要直接在对象上调用 `Object.prototypes` 上的方法。`eslint`: [no-prototype-builtins](https://eslint.org/docs/rules/no-prototype-builtins)
  ...
  // 错误 => Uncaught TypeError: obj.hasOwnProperty is not a function
...
- 2.5.2.【强制】不要在块中使用函数声明。`eslint`: [no-inner-declarations](https://eslint.org/docs/rules/no-inner-declarations)
  ...
  ```javascript
  // 错误 - 函数声明不是块作用域而是函数作用域，因此在块外也能使用函数，容易引起误解
  if (true) {
    function test() {
      console.log('test');
    }
...
- 2.5.5.【推荐】箭头函数编码风格。`eslint`: [arrow-parens](https://eslint.org/docs/rules/arrow-parens) [arrow-body-style](https://eslint.org/docs/rules/arrow-body-style)
  ...
  - 函数体风格
    ...
    ```javascript
    // 正确 - 函数体包含多条语句时，始终加上大括号
    [1, 2, 3].map((number) => {
      const nextNumber = number + 1;
      return `A string containing the ${nextNumber}.`;
    });

    // 正确 - 函数体只包含一条 `return` 语句时，可以也建议省略大括号和 `return`
    [1, 2, 3].map(number => `A string containing the ${number + 1}.`);

    // 正确 - 也可以选择始终不省略大括号，不使用简写语法糖，以方便后续在函数体内增加语句
    [1, 2, 3].map((number) => {
  ...
  - 函数参数风格
    ...
    ```javascript
    // 正确 - 未使用 return 简写语法时，参数始终加上小括号
    [1, 2, 3].map((number) => {
      const nextNumber = number + 1;
      return `A string containing the ${nextNumber}.`;
    });

    // 正确 - 使用 return 简写语法、且只有一个参数时，可以也建议省略参数的小括号
    [1, 2, 3].map(x => x * x);

    // 正确 - 也可以选择始终不省略参数的小括号，以方便后续可能要增加参数
    [1, 2, 3].map((x) => x * x);
    ```
...
- 2.8.1.【推荐】使用严格相等运算符。eslint: [eqeqeq](https://eslint.org/docs/rules/eqeqeq)
  ...
  // 错误 - 为了兼容 id 可能是字符串的情况，而有意使用 == 与数字比较
  if (id == 83949) {
    // do something
  }

  // 正确 - 如果 id 可能是字符串，应该先将其进行类型转换，再使用 === 进行比较
  if (Number(id) === 83949) {
    // do something
  }
  ```
...
- 2.9.2.【推荐】`switch` 语句需要始终包含 `default` 分支。`eslint`: [default-case](https://eslint.org/docs/rules/default-case)
  ...
  // 正确 - 如果有意省略 default，请在 switch 语句末尾用 `// no default` 注释指明
  let foo = 0;
  switch (bar) {
    case 1:
      foo = 2;
...
- 2.10.4.【推荐】生产环境禁止使用 `console`。eslint: [no-console](https://eslint.org/docs/rules/no-console)
  ...
  // 正确 - 如果你非要使用 console 语句，可以考虑自己进行封装以确保不要在生产环境暴露调试信息
  const utils = {
    log: (msg) => {
...
- 4.4.【参考】全部大写字母&单词间用下划线分割的命名模式（`UPPERCASE_VARIABLES`）。
  ...
  ```javascript
  // 错误 - 在本文件中使用的常量，不需使用 UPPERCASE_VARIABLES 风格
  ...
  ```javascript
  // 错误 - unnecessarily uppercases key while adding no semantic value
